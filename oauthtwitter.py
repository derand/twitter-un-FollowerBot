...
class OAuthApi:
    ...
    def autorized(self, options={}):
        rv = True
        try:
            json = self._FetchUrl("https://api.twitter.com/1/statuses/user_timeline.json", "GET", options)
        except urllib2.HTTPError, e:
            rv = False
        return rv
 
    def searchByQuery(self, query, options={}):
       options['q'] = query
       return self.ApiCall("users/search", "GET", options)
       
    def ApiCall(self, call, method="GET", parameters={}, outType='json'):
       ...
       Args:
        ...
        try:
            json = self._FetchUrl("https://api.twitter.com/1/" + call + ".json", method, parameters)
